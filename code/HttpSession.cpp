...
void HttpSession::HttpProcess(const HttpRequestContext &httprequestcontext, std::string &responsecontext)
{
    ...
    //std::cout<<"body : "<< httprequestcontext.body <<std::endl;
    ...
    {
        path = "/NetServer/register.html";
    }
    else if("/1" == path){
        path = "/NetServer/log.html";
    }
    else if("/5" == path){
        path = "/NetServer/picture.html";
    }
    else if("/6" == path){
        path = "/NetServer/video.html"; 
    }
    else if("/7" == path){
        path = "/NetServer/hello.html"; 
    }
    else if("/2" == path || "/3" == path){
        std::string m_string = httprequestcontext.body;
        //将用户名和密码提取出来
        //user=123&passwd=123
        ...
        password[j] = '\0';
        std::cout<<"get user : "<<name<<' '<<" and password : "<<password<<std::endl;
        (std::string)name;
        std::string value = skiplist.search_element(name);
        //register校验
        if ("/3" == path){
            std::cout<<"path == /3"<<std::endl;
            if(value == "-1"){
                path = "/NetServer/log.html";
                skiplist.insert_element(name, password);
                skiplist.upload_data(); 
            }
            else{
                path = "/NetServer/registerError.html";
            }
        }//log校验
        else{
            if(password == value){
                path = "/NetServer/welcome.html";
            }
            else{
                path = "/NetServer/logError.html";
            }
        }
    }
    else{
    ...
    {
        char buffer[4096];int i = 0;
        memset(buffer, 0, sizeof(buffer));
        while(fread(buffer, sizeof(buffer), 1, fp) == 1)//可以mmap内存映射优化
        {
            responsebody.append(buffer);//std::cout<<i++<<' '<<responsebody.size()<<std::endl;
            memset(buffer, 0, sizeof(buffer));
        }
        if(feof(fp)) 
        {
